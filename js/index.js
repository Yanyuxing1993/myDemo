...
function navbarResize(){
    ...
    if($("#sidebarSwitch").hasClass("open")){

        $("#navbar").css("min-width",allWidth - 240 + 'px');
...
$("#sidebarSwitch").on("click", function () {
    ...
    if ($(this).hasClass("open")) {
        ...
        if (window.innerWidth > 768) {
            ...
            var navbarW = parseInt($('#navbar').css('width'));

            $('#navbar').css('width', (navbarW - 240) + 'px');
        }
...
$('.menu').on('click', function (e) {
    ...
    var menuSwc = $(this);      //被点击的菜单开关

    var i = menuSwc.attr('href');   //跳转目标的id

    var navbarHeight = $('#navbar').height();

    var boxTop = $(i).offset().top - navbarHeight;
    /* 目标元素距离文档顶部的距离*/
...
//二级菜单被点击的事件处理
//动画效果与一级菜单一样
$("a[class*='switch']").on('click', function (e) {
...
$(window).on('scroll', function () {
    ...
    var docTop = $(document).scrollTop();   //当前滚动过的距离
    var startTop;   //元素顶部到页面顶部的距离
    var endTop;     //元素底部到页面顶部的距离
    var menuList = $("a[class*='switch']");

    for (var i = 0; i < menuList.length; i++) {

        var elemID = menuList[i].getAttribute('href');

        var elem = $(elemID);

        //导航栏的高度

        startTop = elem.offset().top - 200;
        console.log("测试代码报错原因：页面未完成，侧边栏找不到对应元素");

        endTop = startTop + parseInt(elem.css('height'));

        if (docTop >= startTop && docTop <= endTop) {

            var active = $(elem.data('switch'));
            //移除其他的class
            $('a.active').removeClass('active');
            //焦点开关自身增加class
            active.addClass('active');
            //焦点元素的父元素ul显示block
            active.parents('ul.menu-list-2').slideDown();
            //移除其他的class
            if (active.parents('.menu-list-2').prev().hasClass('menu-open')) {
                return;
            } else {

                $('.menu-open').next('ul.menu-list-2').hide();

                $('.menu-open').removeClass('menu-open');
                active.parents('.menu-list-2').prev().addClass('menu-open')
 ...
 * */
//关闭功能按钮
$('.btn-close').on('click', function (e) {      //面板关闭按钮功能
 ...
 * */
//表格
$.getJSON("data/tableGoods.php", function (json) {
    ...
    for (var i = 0; i < json.length; i++) {
        ...
        var tr = $('<tr></tr>');
        tr.append('<td>' + json[i].gid + '</td>');
        tr.append('<td>' + json[i].price + '</td>');
        tr.append('<td>' + json[i].buy + '</td>');
        tr.append('<td>' + json[i].laud +'<a href="#" class="radar-switch">查看详情</a>'+ '</td>');
        tr.data('url', json[i].url + '');
        tbody.append(tr);
    }

    $('#Goodscontent').append(tbody);
    tbody.children('tr:first-child').addClass('click');

    tbody.on('click', 'tr', function () {
        //前一张图片隐藏
        var oldURL = $(".click").data("url");
        $(".click").removeClass("click");
        //后一张图片显示
        $(this).addClass('click');
        var img = $('<img/>');
        img.attr('src', $(this).data('url'));
        var d = $('<div></div>').addClass('banner-box').hide();
        d.append(img);
        $("#banner>div").fadeOut('slow', function () {//渐隐动画完毕后删除该元素

            $(this).remove();

        });
        $("#banner").append(d);
        $("#banner>div").fadeIn('slow');
    ...
    (function () {
        var timer = setInterval(function () {
            ...
            if(next[0]){
                ...
                $(".click").parent().children().first().trigger("click");
        ...
        var img = $('<img/>');
        img.attr('src', $(".click").data('url'));
        var d = $('<div></div>').addClass('banner-box');
        d.append(img);
        $("#banner").append(d);
    })();
...
//AJAX获取后台数据并绘图
function pieAJAX(dataURL, table, pieTitle) {

    $.getJSON('data/' + dataURL + '.php', function (json) {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            /*TODO 可简化操作*/
            var tr = $('<tr></tr>');
            tr.html('<td>' + json[i].vName + '</td><td>'
            + json[i].IP + '</td><td>'
            + json[i].toBuy + '</td><td>'
            + json[i].f + '</td>');

            //存储绘图所需数据
            lData[i] = json[i].vName;
            data[i] = {value: json[i].IP, name: json[i].vName}
            //挂载表格数据
            tbody.append(tr);
...
//加载成功时自动绘图
pieAJAX($('.tab-active').data('url'), $('#visitConten'), $('.tab-active').html() + '流量构成图');


//tab PC & 手机端 切换按钮
$('.panel-tab > a').click(function (e) {
    ...
    var pieTitile = $(this).html() + '流量构成图';

    var table = $($(this).attr('href'));

    table.find('tbody').remove();       //删除元素

    var dataURL = $(this).data('url');
    //发送AJAX请求刷新数据
    pieAJAX(dataURL, table, pieTitile);
...
var visit = echarts.init(document.getElementById('chartVisit'));
    ...
    series: [        //数据配置
        {
            name: '访客数',
            type: 'bar',
            data: [1700, 2565, 2658, 5445, 4545, 4961, 5514]
        },
        {
            name: '浏览量',
            type: 'bar',
            data: [3720, 4554, 4542, 11251, 14564, 8000, 7566]
        }
    ],
    grid: {                          //绘图范围
        x: 80,
...
function randomData() {
    ...
    //均价
    var average = Math.floor(Math.random() * 30) + 30;
    //时间轴
    var time = new Date().toLocaleTimeString();
    //总计金额
    var total = number * average;
    rateVal += total;
    return {
        time: time,
        total: total,
...
//实例化图表
var sales = echarts.init(document.getElementById('chartSale'));
...
sales.setOption(option);        //销售情况大图

/*
 *                   rate-完成情况比率图
 * */
var rate = echarts.init(document.getElementById('chartRate'));
    ...
    series: [
        {
            ...
            data: [
                {
                    value: parseInt(rateVal / 10000) * 100
                }
            ]
        }
    ],
...
//初始化图表数据
//rate.setOption(rateOption);     //完成量统计
//sales.setOption(option);        //销售情况大图
visit.setOption(visitOp);       //流量柱状大图
// pie.setOption(pieOp);            //饼状图
//hotCity.setOption();            //热门城市Map图

//图表尺寸自适应,Echarts重绘API
$(window).on('resize', function () {
    navbarResize();             //导航栏自适应
    rate.resize();              //仪表盘重绘
    sales.resize();             //访问量重绘
